...
//Define constants for problem (Neumann boundary values)
 double h=1000000.0; //EDIT
//Define constants for problem (area and forcing term)
 double area = 0.0001;
 double modulus = 100000000000.;
 double force (double x)
{
	return 100000000000.*x;
} 
...
class FEM
{
 public:
  ...
  //Data structures
  SparsityPattern       sparsity_pattern; //Sparse matrix pattern , it is about K matrix
  ...
  double                basisFunctionOrder, prob, L, g1, g2;    //prob stores the order of lagrangian order in the problem 1 or 2. g1 and g2 are boundary values. L is the length of the domain
...
:
fe (FE_Q<dim>(order), dim), // FE_Q keeps tracks the order of what lagrangian functions are being used. while declaring this function, we call data object
...
{
if (node==0)
	value = (xi-1)*(xi+1./3.)*(xi-1./3.)/((-1.-1./3.)*(-1.+1./3.)*(-1.-1.));
if (node==1)
	value = (xi+1.)*(xi-1./3.)*(xi+1./3.)/((1.-1./3.)*(1.+1./3.)*(1.+1.));
if (node==2)
	value = (xi-1.)*(xi+1.)*(xi-1./3.)/((-1./3.-1./3.)*(-1./3.-1)*(-1./3.+1.));
if (node==3)
	value = (xi+1./3.)*(xi-1.)*(xi+1.)/((2./3.)*(1./3.-1.)*(1./3.+1.));
...
template <int dim> // this is derivative about xi.
...
{
if (node==0)
	value = -0.5+xi;  //-xi*(1.0-xi)/2.0;
if (node==1)
	value = 0.5+xi;  //xi*(1.0+xi)/2.0;
if (node==2)
	value = -2*xi; //1.0-xi*xi;
}

if (basisFunctionOrder == 3)
{
if (node==0)
	value = (-1.6875*xi*xi+1.125*xi+0.0625); //(xi+0.33333)*(xi-0.33333)*(xi-1.0)/(-1.777778);
if (node==1)
	value =(1.6875*xi*xi+1.125*xi-0.0625); //(xi+0.33333)*(xi-0.33333)*(xi+1.0)/(1.777778);
if (node==2)
	value = 5.0625*xi*xi-1.125*xi-1.6875; //(xi+1.0)*(xi-0.33333)*(xi-1.0)/(0.592593);
if (node==3)
	value = -5.0625*xi*xi-1.125*xi+1.6875; //(xi+1.0)*(xi+0.33333)*(xi-1.0)/(-0.592593);
}
...
void FEM<dim>::generate_mesh(unsigned int numberOfElements){
  ...
  Point<dim,double> min(x_min), // this is deal.ii function, which is location vector.
    max(x_max);
...
void FEM<dim>::setup_system(){
  ...
  define_boundary_conds(); //problemObject.define_boundary_conds();
...
quadRule = 3; //EDIT - Number of quadrature points along one dimension
  ...
  quad_weight[0] = 0.555556; //EDIT
  quad_weight[1] = 0.888889; //EDIT
  quad_weight[2] = 0.555556;
...
void FEM<dim>::assemble_system(){
  ...
  FullMatrix<double> 				Klocal (dofs_per_elem, dofs_per_elem); // fullmatrix is another deal.ii object.
  Vector<double>      			Flocal (dofs_per_elem);
  std::vector<unsigned int> local_dof_indices (dofs_per_elem);
  double										h_e, x, f, x_grad;
  ...
  for (;elem!=endc; ++elem){
    ...
    elem->get_dof_indices (local_dof_indices);//-> This is how we access the function, or elem.get_dof_indices (local_dof_indices);
	...
	for(unsigned int B=0; B<dofs_per_elem; B++){
	  x += nodeLocation[local_dof_indices[B]]*basis_function(B,quad_points[q]);
		...
		Flocal[A] += (area*h_e/2.0)*basis_function(A,quad_points[q])*force(x)*quad_weight[q];//EDIT - Define Flocal.
	...
	Flocal[dofs_per_elem-1] += h;//EDIT - Modify Flocal to include the traction on the right boundary.
      }
    }
	...
	Klocal[A][B] +=(2.*modulus*area/h_e)*basis_gradient(A,quad_points[q])*basis_gradient(B,quad_points[q])*quad_weight[q] ; //EDIT - Define Klocal.
	}
      }
    }
...
void FEM<dim>::solve(){
  ...
  A.initialize(K);// initialize global matrix K
  A.vmult (D, F); //D=K^{-1}*F
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
	...
	u_h += D[local_dof_indices[B]]*basis_function(B,quad_points[q]);
      ...
      //EDIT - Find the l2-norm of the errorR through numerical integration.	
	if (prob==1){
 	 u_exact=-(force(1)/(6.0*modulus))*x*x*x+(g2+force(1)*L*L*L/(6*modulus)-g1)/L*x+g1;
	}
	if (prob==2){
 	 u_exact=-(force(1)/(6.0*modulus))*x*x*x+(-h-force(1)*area*L*L/2.0)*x+g1;
	}
       l2norm+=(u_exact-u_h)*(h_e/2.)*quad_weight[q];
