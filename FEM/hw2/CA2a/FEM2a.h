...
class FEM
{
 public:
  ...
  double Ly; // this is user defined variable
  //solution name array
...
FEM<dim>::FEM ()
:
fe (FE_Q<dim>(1), 1),  // this means linear basis and 1 degree of freedom
...
const unsigned int totalNodes = dof_handler.n_dofs(); //Total number of nodes
  ...
  for(unsigned int globalNode=0; globalNode<totalNodes; globalNode++){
    if(nodeLocation[globalNode][1] == 0){
      boundary_values[globalNode] = 300*(1+0.33333*nodeLocation[globalNode][0]);
    }
    if(nodeLocation[globalNode][1] == Ly){
      
	boundary_values[globalNode] = 310*(1+8*nodeLocation[globalNode][0]*nodeLocation[globalNode][0]);
...
void FEM<dim>::setup_system(){
  ...
  std::cout << basis_gradient(0, 1, 0)[0] << std::endl;
  std::cout << basis_gradient(0, 1, 0)[1] << std::endl;
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
	...
	for(unsigned int A=0; A<dofs_per_elem; A++){
	// Flocal[A] += basis_function(A, quad_points[q1], quad_points[q2])*force(quad_points[q1], quad_points[q2]);//You would define Flocal here if it were nonzero.
	}
      }
    }
	  ...
	  for(unsigned int B=0; B<dofs_per_elem; B++){
		...
		for(unsigned int I=0;I<dim;I++){
		  for(unsigned int J=0;J<dim;J++){
		    ...
		    Klocal[A][B]+=-basis_gradient(A,quad_points[q1],quad_points[q2])[I]*invJacob[I][i]*kappa[i][j]*basis_gradient(B,quad_points[q1],quad_points[q2])[J]*invJacob[J][j]*detJ*quad_weight[q1]*quad_weight[q2] ;//EDIT - Define Klocal. You will need to use the inverse Jacobian ("invJacob") and "detJ"
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      //F[local_dof_indices[A]]+=Flocal[A];//You would assemble F here if it were nonzero.
      for(unsigned int B=0; B<dofs_per_elem; B++){
