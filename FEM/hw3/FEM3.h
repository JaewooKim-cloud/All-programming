...
void FEM<dim>::define_boundary_conds(){
 ...
 for(unsigned int globalNode=0; globalNode<totalDOFs; globalNode++){
	double nodalDOF = globalNode%dim;
    if(dofLocation[globalNode][2] == 0 && nodalDOF == 0){// this corresponds 3 dofs.
      boundary_values[globalNode] = 0.0; 
    }
    if(dofLocation[globalNode][2] == 0 && nodalDOF == 1){// this corresponds 3 dofs.
      boundary_values[globalNode] = 0.0; 
    }
    if(dofLocation[globalNode][2] == 0 && nodalDOF == 2){// this corresponds 3 dofs.
      boundary_values[globalNode] = 0.0; 
    }
...
void FEM<dim>::assemble_system(){
  ...
  FEValues<dim> fe_values (fe,
			   ...
			   update_values | 
			   update_gradients | 
			   update_JxW_values); // J stands for det of jacobian and w means quadrature weight.
  ...
  for (;elem!=endc; ++elem){
	...
	of the current element. We can use it to see if we are at the Neumann boundary, x_3 = 1.*/
      if(elem->face(f)->center()[2] == 1){ // this is to check whether it is neumann
	...
	h[0]=0.0, h[1]=0.0, h[2]=1.0*x;	//EDIT - define the value of the traction vector, h
	  for (unsigned int A=0; A<nodes_per_elem; A++){ //loop over all element nodes
		...
		For det(J) times the total quadrature weight: fe_face_values.JxW(q)*/                                    		Flocal[dim*A+i]+=h[i]*fe_face_values.shape_value(dim*A+i,q)*fe_face_values.JxW(q);
	    }
	  }
	}
      }
