...
for(unsigned int globalNode=0; globalNode<totalNodes; globalNode++){
    if(nodeLocation[globalNode][0] == 0){// At x1=0.0, u = 300K, du/dt=0
      boundary_values_of_D[globalNode] = 300.0;
      boundary_values_of_V[globalNode] = 0.0; 
    }
    if(nodeLocation[globalNode][0] == 1.0){// At x1=1.0, u = 310K, du/dt=0
...
void FEM<dim>::assemble_system(){
  ...
  double		    rho = 3.8151*pow(10.0,6.0);                            //EDIT - specify the specific heat per unit volume
...
//Klocal[A][B]+=-basis_gradient(A,quad_points[q1],quad_points[q2],quad_points[q3])[I]*invJacob[I][i]*kappa[i][j]*basis_gradient(B,quad_points[q1],quad_points[q2],quad_points[q3])[J]*invJacob[J][j]*detJ*quad_weight[q1]*quad_weight[q2]*quad_weight[q3]
...
void FEM<dim>::solve_trans(){
  ...
  for(unsigned int t_step=1; t_step<5001; t_step++){
  ...
  system_matrix.copy_from(M); // system_matrix=M
  system_matrix.add(alpha*delta_t,K); // M+alpha*delta_t*K 
  K.vmult(RHS,D_tilde); //RHS = K*D_tilde
  RHS *= -1.; 		//RHS = -1.*RHS = -K*D_tilde

  RHS.add(1.,F); 	//RHS = RHS + 1.*F = F - K*D_tilde
