program mc_npt
    ...
    implicit none
    integer,parameter :: seed = 1285
    integer, parameter :: npart = 100, nsteps = 20000, printFreq = 100
    real*8 :: density , temp, beta, vmax,ra,maxDr
    real*8, dimension (3,npart) :: coords
    integer i,j
    real*8 :: L,deltaE, pressure, energy
   ...
   call initcubicgrid(npart,pressure,density,L,coords,temp) ! set initial configuraion
   ...
   ! mc cycle
    do i=1, nsteps
        do j=1, npart
            ra = rand()*(npart+1)+1
            if (ra.le.npart) then ! it is mc movr criterion
               ...
               call pbc3d (rtrial,L) ! apply periodic boundary condition
                ...
                call mcvol(L,energy,vmax,npart,coords,pressure,beta)
                density = npart/(L**3)
                energy = LJ_Energy(coords,L,npart,density)
             end if
        end do
        if (i.gt.10000) then
        pressure1(i-10000) = density/beta + vir(npart,coords,L)/(L**3)
        pressure1(i-10000) = pressure1(i-10000) + 16*3.141592*(1.0/3.0)*(density**2.0)*((2.0/3.0)*((0.5*L)**(-9.0))-(0.5*L)**(-3.0))
        end if
    end do
...
subroutine initcubicgrid(npart,pressure,density,L,coords,temp)

    implicit none
    real*8, dimension (3,npart), intent(inout) :: coords
    real*8, intent(in) :: pressure,temp
...
subroutine mcvol (Lo,eno,vmax,npart,coords,p,beta)

    implicit none
    real*8 :: vo,lnvn,vn,Ln,densityn,arg,enn
    real*8, intent(inout) :: Lo,eno
    real*8, intent(in) :: vmax,p,beta
    integer, intent(in) :: npart
    real*8, dimension(3,npart), intent(inout) :: coords
    ...
    vo = Lo**3
    lnvn = log(vo)+(rand()-0.5)*vmax ! perform random walk in lnV
    vn = exp(lnvn)
    Ln = vn**(1.0/3.0) ! new box lenghth
    do i=1,npart
        coords(:,i) = coords(:,i)*Ln/Lo !rescale center of mass
    end do
    
    densityn = npart/vn
    
    enn = LJ_Energy(coords,Ln,npart,densityn)
    arg = -beta*((enn-eno)+p*(vn-vo)-(npart+1)*(log(vn/vo))*(1.0/beta))
    
    
    if (rand().gt.exp(arg)) then
        do i=1,npart
            coords(:,i) = coords(:,i)*Lo/Ln !rescale center of mass
        end do
    else
        eno = enn
        Lo = Ln
